...
def save_snapshot(submit_config, net, fname_postfix):
    dump_fname = os.path.join(
        submit_config.run_dir, "network-%s.pickle" % fname_postfix
    )
...
def compute_ramped_lrate(
    i, iteration_count, ramp_up_fraction, ramp_down_fraction, learning_rate
):
...
def load_datasets(
    num_channels, dataset_dir, train_dataset, validation_dataset, prune_dataset=None
):
    # Training set.
    ...
    if train_dataset is None:
        ...
        for i in range(0, num, bs):
            ...
            for j in range(n):
                train_images[i + j] = set_color_channels(
                    np.reshape(img[j], shp[j]), num_channels
                )
    ...
    if validation_dataset in ["kodak", "bsd300", "set14"]:
        paths = {
            "kodak": os.path.join(dataset_dir, "kodak", "*.png"),
            "bsd300": os.path.join(
                dataset_dir, "BSDS300", "images/test/*.jpg"
            ),  # Just the 100 test images
        ...
        validation_images = [
            x[..., np.newaxis] if len(x.shape) == 2 else x for x in validation_images
        ]  # Add channel axis to grayscale images.
        validation_images = [x.transpose([2, 0, 1]) for x in validation_images]
        validation_images = [
            set_color_channels(x, num_channels) for x in validation_images
        ]  # Enforce RGB/grayscale mode.
        print("Loaded %d images." % len(validation_images))

    # Pad the validation images to size.
    validation_image_size = [
        max([x.shape[axis] for x in validation_images]) for axis in [1, 2]
    ]
    validation_image_size = [
        (x + 31) // 32 * 32 for x in validation_image_size
    ]  # Round up to a multiple of 32.
    validation_image_size = [
        max(validation_image_size) for x in validation_image_size
    ]  # Square it up for the rotators.
    print(
        "Validation image padded size = [%d, %d]."
        % (validation_image_size[0], validation_image_size[1])
    )
...
def analysis_network(image, num_output_components, blindspot, zero_last=False):
        ...
        with tf.variable_scope(name):
            ...
            W = tf.get_variable(
                ...
                initializer=(
                    ...
                    if zero_weights
                    else tf.initializers.random_normal(0.0, wstd)
                ),
            )
            b = tf.get_variable("b", shape=[n_out], initializer=tf.initializers.zeros())
            if ofs > 0:
                n = tf.pad(n, [[0, 0], [0, 0], [ofs, 0], [0, 0]])
            n = tf.nn.conv2d(
                n, W, strides=[1] * 4, padding="SAME", data_format="NCHW"
    ...
    def rotate(x, angle):
        if angle == 0:
            ...
            return tf.transpose(x[:, :, :, ::-1], [0, 1, 3, 2])
        elif angle == 180:
            return x[:, :, ::-1, ::-1]
        elif angle == 270:
            return tf.transpose(x[:, :, ::-1, :], [0, 1, 3, 2])
    ...
    if blindspot:
        ...
        x = conv(
            x, "nin_c", num_output_components, size=1, gain=1.0, zero_weights=zero_last
        )
    else:
...
def blindspot_pipeline(
    ...
    **_kwargs
    ...
    if noise_params == "global":
        ...
        noise_est_out = tf.get_variable(
            "noise_data", shape=[1, 1, 1, 1], initializer=tf.initializers.constant(0.0)
        )  # 1111
        ...
        with tf.variable_scope("param_estimation_net"):
            noise_est_out = analysis_network(
                noisy_in, 1, blindspot=False, zero_last=True
            )  # N1HW
        noise_est_out = tf.reduce_mean(
            noise_est_out, axis=[2, 3], keepdims=True
        )  # N111
        noise_est_out = tf.cast(noise_est_out, tf.float64)
    ...
    if noise_style == "gauss":
        ...
        if noise_params == "known":
            noise_std = (
                tf.maximum(mu_x, tf.constant(1e-3, tf.float64)) / noise_params_in
            ) ** 0.5  # NCHW
        else:
            noise_std = (
                tf.maximum(mu_x, tf.constant(1e-3, tf.float64)) * noise_est_out
            ) ** 0.5  # NCHW
    ...
    def batch_vtmv(v, m):  # Batched (v^T * M * v).
        return tf.reduce_sum(
            v[..., :, tf.newaxis] * v[..., tf.newaxis, :] * m, axis=[-2, -1]
        )
    ...
    if noise_style in ["gauss", "poisson"]:
        if num_channels == 1:
            ...
            # Training loss.
            sigma_n = (
                tf.transpose(noise_std**2, [0, 2, 3, 1])[..., tf.newaxis] * I
            )  # NHWC1 * NHWCC = NHWCC
            sigma_y = (
                sigma_x + sigma_n
            ...
            if noise_params != "known":
                loss_out = loss_out - 0.1 * tf.reduce_mean(
                    noise_std, axis=1
            ...
            # Summary statistics.
            net_std_out = tf.maximum(zero64, tf.linalg.det(sigma_x)) ** (
                1.0 / 6.0
            )  # NHW
            noise_std_out = tf.maximum(zero64, tf.linalg.det(sigma_n)) ** (
                1.0 / 6.0
            )  # N11 / NHW
    elif noise_style == "impulse":
        ...
        if num_channels == 1:
            ...
            dets = tf.maximum(
                tf.constant(1e-9, dtype=tf.float64), dets
            )  # NHW. Avoid division by zero and negative square roots.
            g = (
                tf.exp(-0.5 * diff) / ((2.0 * np.pi) ** num_channels * dets) ** 0.5
            )  # NHW
            g = g[..., tf.newaxis]  # NHW1
            ...
            mu_y2 = alpha * 0.5 + (1.0 - alpha) * mu_x2  # NHWC
            alpha = alpha[..., tf.newaxis]  # n1111
            sigma_y = (
                alpha * (1.0 / 4.0 + I / 12.0)
                + (1.0 - alpha) * (sigma_x + batch_vvt(mu_x2))
                - batch_vvt(mu_y2)
            )  # NHWCC
            sigma_y_inv = tf.linalg.inv(sigma_y)  # NHWCC
            ...
            net_std_out = tf.maximum(zero64, tf.linalg.det(sigma_x)) ** (
                1.0 / 6.0
            )  # NHW. Cube root of volumetric scaling factor.
            noise_std_out = (
                alpha[..., 0, 0] / 255.0 * 100.0
            )  # N11 / NHW. Shows as percentage in output.
...
def simple_pipeline(
    ...
    **_kwargs
    ...
    if (
        noise_style == "impulse" and noisy_targets
    ):  # Cannot use L2 loss because mean changes
...
def noisify(x, style, param_1, param_2):
    ...
    if style.startswith("gauss"):  # Gaussian noise with constant/variable std.dev.
        ...
        "impulse"
    ):  # Random replacement with constant/variable alpha.
        params = [float(p) * 0.01 for p in style.replace("impulse", "", 1).split("_")]
        msh = tf.shape(x[:, :1, ...])
        if (param_1 > -0.5) and (param_2 < 0):
            alpha = param_1
            keep_mask = tf.where(
                tf.random_uniform(shape=msh, seed=get_seed()) >= alpha,
                tf.ones(shape=msh),
                tf.zeros(shape=msh),
            )
            ...
            keep_mask = tf.where(
                tf.random_uniform(shape=msh, seed=get_seed())
                >= tf.ones(shape=msh) * alpha,
                tf.ones(shape=msh),
                tf.zeros(shape=msh),
            )
        elif len(params) == 1:
...
def train(
    ...
    # Load the data.
    train_images, validation_images, validation_image_size = load_datasets(
        num_channels,
        dataset_dir,
        None if eval_mode else train_dataset,
        validation_dataset,
        prune_dataset,
    )

    # Repeat validation set if asked to.
    original_validation_image_count = len(
        validation_images
    )  # Avoid exporting the duplicate images.
    ...
    with tf.device("/gpu:0"):
        if eval_mode:
            ...
            if noise_style.startswith("gauss"):
                net_noise_style = "gauss"
            if noise_style.startswith("poisson"):
                net_noise_style = "poisson"
            if noise_style.startswith("impulse"):
                net_noise_style = "impulse"

            if pipeline == "blindspot":
                net = dnnlib.tflib.Network(
                    "net",
                    "selfsupervised_denoising.blindspot_pipeline",
                    input_shape=input_shape,
                    noise_style=net_noise_style,
                ...
                net = dnnlib.tflib.Network(
                    "net",
                    "selfsupervised_denoising.simple_pipeline",
                    input_shape=input_shape,
                    noise_style=net_noise_style,
                    blindspot=True,
                    noisy_targets=True,
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.device("/gpu:%d" % gpu):
            ...
            noisy_in_gpu, noise_coeff = noisify(
                clean_in_gpu, noise_style, param_1, param_2
            )

            if pipeline == "blindspot_mean":
                reference_in_gpu = noisy_in_gpu
            elif pipeline == "n2n":
                reference_in_gpu, _ = noisify(
                    clean_in_gpu, noise_style, param_1, param_2
                )  # Another noise instantiation.
            else:
                reference_in_gpu = clean_in_gpu

            noise_coeff = (
                tf.zeros([tf.shape(noisy_in_gpu)[0], 1, 1, 1]) + noise_coeff
            )  # Broadcast to [n, 1, 1, 1] shape.
            ...
            if net.num_inputs == 5:
                (
                    ...
                    noise_std_out,
                    _,
                ) = net_gpu.get_output_for(
                    reference_in_gpu,
                    noisy_in_gpu,
                    noise_coeff,
                    tf.constant(1e-6, dtype=tf.float32),
                    tf.constant(1e-1, dtype=tf.float32),
                )
            else:
                if pipeline == "blindspot":
                    if net.num_inputs == 3:
                        (
                            ...
                            noisy_in_gpu, noise_coeff, L_exponent_in
                        )  # Previous version.
                    else:
                        (
                            mu_x,
                            ...
                            net_std_out,
                            noise_std_out,
                        ) = net_gpu.get_output_for(noisy_in_gpu, noise_coeff)
                else:
                    if net.num_inputs == 4:
                        (
                            mu_x,
                            ...
                            reference_in_gpu, noisy_in_gpu, noise_coeff, L_exponent_in
                        )  # Previous version.
                    else:
                        (
                            mu_x,
                            ...
                            reference_in_gpu, noisy_in_gpu, L_exponent_in
                        )

            gpu_outputs.append(
                [mu_x, pme_out, loss_out, net_std_out, noise_std_out, noisy_in_gpu]
            )
            ...
            psnr_pme = tf.reduce_mean(
                calculate_psnr(pme_out, clean_in_gpu, axis=[1, 2, 3])
            )
            with tf.control_dependencies(
                [
                    autosummary("train_loss", loss),
                    autosummary("train_psnr", psnr),
                    autosummary("train_psnr_pme", psnr_pme),
                ]
            ):
    ...
    # Total outputs.
    mu_x_out, pme_out, loss_out, net_std_out, noise_std_out, noisy_out = [
        tf.concat(x, axis=0) for x in zip(*gpu_outputs)
    ]
    ...
    loss_acc, loss_n = 0.0, 0.0
    psnr_acc, psnr_pme_acc = 0.0, 0.0
    std_net_acc, std_noise_acc = 0.0, 0.0
    valid_psnr_mu, valid_psnr_pme = 0.0, 0.0
    t_start = time.time()
    ...
    for n in range(0, num_iter + minibatch_size, minibatch_size):
        ...
        if not eval_mode:
            ...
            clean = np.zeros(
                [minibatch_size, num_channels, train_resolution, train_resolution],
                dtype=np.uint8,
            )
        ...
        if n == 0 or n >= print_last + print_interval:
            ...
            print(
                "%8d: time=%6.2f, loss=%8.4f, train_psnr=%8.4f, train_psnr_pme=%8.4f, std_net=%8.4f, std_noise=%8.4f"
                % (
                    n,
                    t_iter,
                    loss_acc,
                    psnr_acc,
                    psnr_pme_acc,
                    autosummary("std_net", std_net_acc),
                    autosummary("std_noise", std_noise_acc),
                ),
                end="",
            )
            ctx.update(
                loss="%.2f %.2f" % (psnr_pme_acc, valid_psnr_pme),
                cur_epoch=n,
                max_epoch=num_iter,
            )
            ...
            if n == 0 or n >= eval_last + eval_interval or n == num_iter:
                ...
                for idx0 in range(0, len(validation_images), bs):
                    ...
                    for i in idx:
                        ...
                        img = np.pad(
                            ...
                            [
                                [0, 0],
                                [0, 0],
                                [0, validation_image_size[0] - sz[0]],
                                [0, validation_image_size[1] - sz[1]],
                            ],
                            "reflect",
                        )
                    ...
                    mu_x, net_std, pme, noisy = tfutil.run(
                        [mu_x_out, net_std_out, pme_out, noisy_out], feed_dict
                    )

                    # Process the result images.
                    for i, j in enumerate(idx[:num]):
                        crop_val_input, crop_mu_x, crop_pme, crop_noisy = [
                            x[i, :, : val_sz[i][0], : val_sz[i][1]]
                            for x in [val_input, mu_x, pme, noisy]
                        ]
                        crop_net_std = net_std[
                            i, : val_sz[i][0], : val_sz[i][1]
                        ]  # HW grayscale
                        crop_net_std /= 10.0 / 255.0  # white = 10 ULPs in U8.
                        valid_psnr_mu += calculate_psnr(
                            crop_mu_x, crop_val_input
                        ) / len(validation_images)
                        valid_psnr_pme += calculate_psnr(
                            crop_pme, crop_val_input
                        ) / len(validation_images)

                        if (eval_mode and (j < original_validation_image_count)) or (
                            (not eval_mode) and (j == len(validation_images) - 1)
                        ):  # Export last image, or all if evaluating.
                            ...
                            def save_img(name, img):
                                save_image(
                                    img,
                                    os.path.join(
                                        img_dir, "img-%07d-%s.%s" % (k, name, ext)
                                    ),
                                    [0.0, 1.0],
                                )
                            ...
                            save_img(
                                "b_out2", crop_pme
                            )  # Posterior mean estimate (actual output)
                ...
                print(
                    ", valid_psnr_mu=%8.4f, valid_psnr_pme=%8.4f"
                    % (valid_psnr_mu, valid_psnr_pme),
                    end="",
                )
...
config_lst = [
    ...
    dict(
        eval_id="00055", noise_style="impulse0_100", pipeline="n2c", num_iter=16000000
    ),
    dict(
        eval_id="00056", noise_style="impulse0_100", pipeline="n2n", num_iter=16000000
    ),
    dict(
        eval_id="00057",
        noise_style="impulse0_100",
        pipeline="blindspot",
        noise_params="known",
        num_iter=4000000,
    ),
...
def main():
    ...
    parser.add_argument(
        "--eval", help="Evaluate validation set with the given network pickle"
    )
    ...
    if args.validation_set not in eval_sets:
        print(
            "Validation set specified with --validation-set not in one of: "
            + ", ".join(eval_sets)
        )
        sys.exit(1)
    ...
    validation_repeats = (
        eval_sets[args.validation_set]["validation_repeats"] if args.eval else 1
    )
    ...
    # Decorate run_desc.
    sc.run_desc += "-ilsvrc"
    if config.get("prune_dataset"):
        sc.run_desc += "_%d" % config.prune_dataset
    sc.run_desc += "-%s" % config.validation_dataset
    sc.run_desc += "-%dc" % config.num_channels
    sc.run_desc += "-%s" % config.noise_style
    if config.minibatch_size != 4:
        sc.run_desc += "-mb%d" % config.minibatch_size
    if config.learning_rate != 3e-4:
        sc.run_desc += "-lr%g" % config.learning_rate
    ...
    if config.get("diagonal_covariance"):
        sc.run_desc += "Diag"
    if config.pipeline == "blindspot":
        sc.run_desc += "-%s" % config.noise_params
    if config.train_resolution != 256:
        sc.run_desc += "-res%d" % config.train_resolution

    if config.get("eval_network"):
        sc.run_desc += "-EVAL_%s" % config_name
    if config.get("eval_network"):
        sc.run_dir_root += "/_eval"
